...
CREATE UNIQUE INDEX idx_unique_carts_per_user ON carts(user_id) WHERE user_id IS NOT NULL;

-- Índice único: Solo 1 carrito activo por sesión guest
CREATE UNIQUE INDEX idx_unique_carts_per_session ON carts(session_id) WHERE session_id IS NOT NULL AND user_id IS NULL;
...
CREATE UNIQUE INDEX idx_unique_addresses_default_per_user ON shipping_addresses(user_id) WHERE is_default = TRUE;
...
CREATE UNIQUE INDEX idx_unique_tokens_default_per_user ON payment_tokens(user_id) WHERE is_default = TRUE;
...
-- Índice único: Solo 1 cuenta B2B por usuario
CREATE UNIQUE INDEX idx_unique_b2b_per_user ON b2b_accounts(user_id);
